...
def astar(p, verbose=False, stepwise=False):
    ...
    if not(p.estBut(bestNoeud.etat)):
        print("Pas de chemin trouvé pour l'instant")
...
def spaceTimeAStar(p, reservations, depth, verbose=False, stepwise=False):
    """
    Application de l'algorithme a-star sur un probleme donné.
    """
    ...
    # if not(p.estBut(bestNoeud.etat)):
    #     print("Pas de chemin trouvé pour l'instant")
    #     reservations[p.init] = True
    #     x,y,t = p.init
    #     reservations[(x,y,t+1)] = True # chaque case est réservée pendant 2 tours pour éviter que 2 joueurs échangent de pos
    #     return [(x,y), (x,y)]

    n = bestNoeud
    path = []
    while n != None :
        reservations[n.etat] = True
        x,y,t = n.etat
        reservations[(x,y,t+1)] = True # chaque case est réservée pendant 2 tours pour éviter que 2 joueurs échangent de pos
        path.append((x,y))
        n = n.pere
